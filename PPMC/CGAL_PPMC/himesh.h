...
enum Hausdorff_Computing_Type { HCT_NULL = 0, HCT_BVHTREE = 1, HCT_ASSOCIATE = 2, HCT_ASSOCIATE_CYLINDER = 3 };
...
class MyMesh : public CGAL::Polyhedron_3<MyKernel, MyItems> {
  ...
  public:
    ...
    MyMesh(string& str, bool completeop = false);
    ...
    Tree* get_aabb_tree();
    float distance(MyMesh* target);
    float distance_tree(MyMesh* target);
    float distance_tree(const Point& p);
    bool intersect(MyMesh* target);
    bool intersect_tree(MyMesh* target);
    ...
    void sample_points(const MyMesh::Face_iterator& fit, unordered_set<Point>& points, float area_unit);
...
class MyTriangle {
  public:
    ...
    // associated
    vector<MyMesh::Face_iterator> facets;
    void add_facet(MyMesh::Face_iterator fit) {
        bool contained = false;
        for (MyMesh::Face_iterator f : facets) {
            if (f == fit) {
                return;
            }
        }
...
enum Decoding_Type { COMPRESSED, RAW };

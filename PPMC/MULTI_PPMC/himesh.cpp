...
bool HiMesh::isRemovable(MCGAL::Vertex* v) {
    //	if(size_of_vertices()<10){
    //		return false;
    //	}
    std::vector<int>& stVertex = stVerteices[i_curDecimationId];
    for (int i = 0; i < stVertex.size(); i++) {
        if (v->poolId == stVertex[i]) {
            return false;
        }
    }
...
std::istream& operator>>(std::istream& input, HiMesh& mesh) {
    ...
    for (std::size_t i = 0; i < mesh.nb_vertices; ++i) {
        ...
        MCGAL::Vertex* vt = MCGAL::contextPool.allocateVertexFromPool(x, y, z, i);
        vt->setId(i);

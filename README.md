...
当前大致算是有了一种非常naive的思路，就是对于buffer的记录翻倍，不仅仅去存储symbol信息，还要去存offset，每个thread读取到某一个offset后，得到的仍是一个offset，然后从这个offset中继续去读
先写个暴力版，对比一下性能和老师汇报一下
周末写完，周一汇报

此时decoding step，create center vertex，joinfacet全都在cuda上了
